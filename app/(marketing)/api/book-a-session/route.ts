import { NextRequest, NextResponse } from 'next/server'
import { sendEmail } from '@/lib/email'
import { business, contato, SITE_URL } from '@/lib/config'
import { FOCUS_AREA_OPTIONS, URGENCY_OPTIONS } from '@/lib/book-session-options'
import { persistFailedBookSessionRequest, sendBookSessionFailureAlert } from '@/lib/book-session-failsafe'

/**
 * Same in-memory rate limiting rationale as app/(marketing)/api/4-week-application/route.ts
 * — single long-lived Node process (next start on Hostinger), not stateless
 * serverless functions.
 */
const rateLimitMap = new Map<string, number[]>()
const RATE_LIMIT_WINDOW_MS = 15 * 60 * 1000
const RATE_LIMIT_MAX = 5

function isRateLimited(ip: string): boolean {
  const now = Date.now()
  const recent = (rateLimitMap.get(ip) || []).filter((t) => now - t < RATE_LIMIT_WINDOW_MS)
  recent.push(now)
  rateLimitMap.set(ip, recent)
  return recent.length > RATE_LIMIT_MAX
}

const ASAP_URGENCY = URGENCY_OPTIONS[0]

function str(body: Record<string, unknown>, key: string): string {
  return typeof body[key] === 'string' ? (body[key] as string).trim() : ''
}

function escapeHtml(value: string): string {
  return value
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;')
}

function nl2br(value: string): string {
  return escapeHtml(value).replace(/\n/g, '<br />')
}

export async function POST(request: NextRequest) {
  const ip = request.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || 'unknown'
  if (isRateLimited(ip)) {
    return NextResponse.json(
      {
        ok: false,
        error: "You've submitted this form too many times in a row. This is temporary — please wait about 15 minutes and try again.",
      },
      { status: 429 }
    )
  }

  let body: Record<string, unknown>
  try {
    body = await request.json()
  } catch {
    return NextResponse.json({ ok: false, error: 'Invalid request.' }, { status: 400 })
  }

  // Honeypot: real visitors never fill this hidden field. Pretend success so
  // bots don't learn the field is being checked.
  if (str(body, 'website') !== '') {
    return NextResponse.json({ ok: true })
  }

  const firstName = str(body, 'firstName')
  const email = str(body, 'email')
  const mobile = str(body, 'mobile')
  const focusArea = str(body, 'focusArea')
  const urgency = str(body, 'urgency')
  const ackAge18 = body.ackAge18 === true
  const ackNonClinical = body.ackNonClinical === true
  const phoneConsent = body.phoneConsent === true

  const errors: string[] = []
  if (!firstName) errors.push('First Name is required.')
  if (!email || !email.includes('@')) errors.push('A valid Email is required.')
  if (!mobile) errors.push('A Mobile Number is required.')
  if (!FOCUS_AREA_OPTIONS.includes(focusArea as (typeof FOCUS_AREA_OPTIONS)[number])) {
    errors.push('Please select a main area of focus.')
  }
  if (!URGENCY_OPTIONS.includes(urgency as (typeof URGENCY_OPTIONS)[number])) {
    errors.push('Please answer how soon you would like to begin.')
  }
  if (!ackAge18 || !ackNonClinical) {
    errors.push('Both required acknowledgements must be confirmed.')
  }

  if (errors.length > 0) {
    return NextResponse.json({ ok: false, error: errors[0], errors }, { status: 400 })
  }

  const submittedAt = str(body, 'submitted_at') || new Date().toISOString()
  const pageUrl = str(body, 'landing_page_url') || `${SITE_URL}/book-a-session/`

  const requestRecord = {
    firstName, email, mobile, focusArea, urgency, phoneConsent, submittedAt, pageUrl,
  }

  if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASSWORD) {
    const reason = 'SMTP_HOST/SMTP_USER/SMTP_PASSWORD not set'
    console.error(`${reason} — cannot send Book a Session request emails.`)
    await persistFailedBookSessionRequest(requestRecord, reason)
    await sendBookSessionFailureAlert(firstName, reason)
    return NextResponse.json(
      { ok: false, error: 'We could not send your request right now. Please try again shortly.' },
      { status: 500 }
    )
  }

  const row = (label: string, value: string) =>
    value ? `<tr><td style="padding:6px 12px 6px 0;color:#555;white-space:nowrap;vertical-align:top;"><strong>${escapeHtml(label)}</strong></td><td style="padding:6px 0;">${nl2br(value)}</td></tr>` : ''

  const notificationHtml = `
    <div style="font-family:Arial,sans-serif;font-size:14px;color:#111;">
      <h2 style="margin-bottom:4px;">New Book a Session request</h2>
      <p style="color:#555;margin-top:0;">${escapeHtml(submittedAt)}</p>
      <table cellpadding="0" cellspacing="0">
        ${row('First Name', firstName)}
        ${row('Email', email)}
        ${row('Mobile', mobile)}
        ${row('Main area of focus', focusArea)}
        ${row('How soon they want to begin', urgency)}
        ${row('Phone contact consent', phoneConsent ? 'Yes' : 'No')}
        ${row('Page URL', pageUrl)}
      </table>
    </div>
  `

  // Confirmation email deliberately never mentions or confirms price — this
  // is a request for review, not a closed sale (see task instructions).
  const confirmationHtml = `
    <div style="font-family:Arial,sans-serif;font-size:15px;color:#111;line-height:1.6;">
      <p>Hi ${escapeHtml(firstName)},</p>
      <p>Thank you for requesting a private session with SAL Ray.</p>
      <p>I review each request personally and will follow up with you directly, by email or phone, within two business days to confirm scheduling and next steps.</p>
      <p>${business.nome}<br />${business.jobTitle}<br /><a href="${SITE_URL}/">${SITE_URL.replace('https://', '')}</a></p>
    </div>
  `

  const asapPrefix = urgency === ASAP_URGENCY ? '[ASAP] ' : ''

  try {
    await sendEmail({
      from: `SAL Ray — Book a Session <${contato.email}>`,
      to: contato.email,
      replyTo: email,
      subject: `${asapPrefix}New Book a Session request — ${firstName}`,
      html: notificationHtml,
    })

    await sendEmail({
      from: `${business.nome} <${contato.email}>`,
      to: email,
      subject: 'We received your session request',
      html: confirmationHtml,
    })
  } catch (err) {
    const reason = err instanceof Error ? err.message : String(err)
    console.error('SMTP send failed for Book a Session request:', err)
    await persistFailedBookSessionRequest(requestRecord, reason)
    await sendBookSessionFailureAlert(firstName, reason)
    return NextResponse.json(
      { ok: false, error: 'We could not send your request right now. Please try again shortly.' },
      { status: 502 }
    )
  }

  return NextResponse.json({ ok: true })
}
